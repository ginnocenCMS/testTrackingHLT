...
TString prefilter = Form("Dgen==23333&&TMath::Abs(Deta)<2.0&&Dtrk1Pt>2.&&Dtrk2Pt>2.&&%s",mvatk.Data());
Bool_t isPbPb = false;
Float_t pthat = 35.;
...
void L1turnon()
{
  ...
  if(isPbPb) 
	  ...
	  infname = Form("/afs/cern.ch/work/j/jisun/public/HFTrig_75x/ppfiles_1116/ntD_20151115_DfinderMC_20151110_EvtMatching_Pythia_D0pt%0.fp0_Pthat%.0f_TuneZ2_5020GeV_GENSIM_75x_1015_20151110_ppGlobaTrackingPPmenuHFlowpuv11_7415_v20_1116.root",pthat,pthat);
  TFile* infile = new TFile(infname);
  TTree* root = (TTree*)infile->Get("ntDkpi");
  TTree* HltTree = (TTree*)infile->Get("HltTree");
  root->AddFriend(HltTree);
  HltTree->AddFriend(root);
